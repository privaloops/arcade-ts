/**
 * Neo-Geo CMC GFX Encryption Tables + Decrypt
 * Translated from MAME prot_cmc.cpp (BSD-3-Clause)
 */

const kof99_type0_t03 = new Uint8Array([
...
const kof99_type0_t12 = new Uint8Array([
...
const kof99_type1_t03 = new Uint8Array([
...
const kof99_type1_t12 = new Uint8Array([
...
const kof99_address_8_15_xor1 = new Uint8Array([
...
const kof99_address_8_15_xor2 = new Uint8Array([
...
const kof99_address_16_23_xor1 = new Uint8Array([
...
const kof99_address_16_23_xor2 = new Uint8Array([
...
const kof99_address_0_7_xor = new Uint8Array([
...
const kof2000_type0_t03 = new Uint8Array([
...
const kof2000_type0_t12 = new Uint8Array([
...
const kof2000_type1_t03 = new Uint8Array([
...
const kof2000_type1_t12 = new Uint8Array([
...
const kof2000_address_8_15_xor1 = new Uint8Array([
...
const kof2000_address_8_15_xor2 = new Uint8Array([
...
const kof2000_address_16_23_xor1 = new Uint8Array([
...
const kof2000_address_16_23_xor2 = new Uint8Array([
...
const kof2000_address_0_7_xor = new Uint8Array([
...
const m1_address_8_15_xor = new Uint8Array([
...
const m1_address_0_7_xor = new Uint8Array([
...
// CMC42 GFX keys (per-game extra_xor)
export const CMC42_KEYS: Record<string, number> = {
  kof99: 0x00, kof99e: 0x00, kof99k: 0x00, kof99p: 0x00,
  garou: 0x06, garouh: 0x06,
  mslug3: 0xAD, mslug3h: 0xAD, mslug3a: 0xAD,
  zupapa: 0xBD, ganryu: 0x07, s1945p: 0x05, preisle2: 0x9F,
  bangbead: 0xF8, nitd: 0xFF, sengoku3: 0xFE,
};

// CMC50 GFX keys
export const CMC50_KEYS: Record<string, number> = {
  kof2000: 0x00, kof2000n: 0x00, kof2001: 0x1E, kof2001h: 0x1E,
  mslug4: 0x31, mslug4h: 0x31, rotd: 0x3F, rotdh: 0x3F,
  pnyaa: 0x2E, kof2002: 0xEC, kof2002b: 0xEC, matrim: 0x6A,
  samsho5: 0x0F, samsho5h: 0x0F, samsh5sp: 0x0D, samsh5sph: 0x0D,
  mslug5: 0x19, mslug5h: 0x19, svc: 0x57, svcsplus: 0x57,
...
function cmcDecryptPair(
  ...
  addrXor: Uint8Array,
  base: number, invert: boolean,
): [number, number] {
  const tmp = table1[(base & 0xFF) ^ addrXor[(base >> 8) & 0xFF]!]!;
  const xor0 = (table0hi[(base >> 8) & 0xFF]! & 0xFE) | (tmp & 0x01);
  const xor1 = (tmp & 0xFE) | (table0lo[(base >> 8) & 0xFF]! & 0x01);
  return invert ? [c1 ^ xor0, c0 ^ xor1] : [c0 ^ xor0, c1 ^ xor1];
}

/** Decrypt C-ROM (sprites) using CMC42 or CMC50 tables */
export function cmcGfxDecrypt(
  rom: Uint8Array, romSize: number, extraXor: number, isCmc50: boolean,
): void {
  const t0_t03 = isCmc50 ? kof2000_type0_t03 : kof99_type0_t03;
  const t0_t12 = isCmc50 ? kof2000_type0_t12 : kof99_type0_t12;
  const t1_t03 = isCmc50 ? kof2000_type1_t03 : kof99_type1_t03;
  const t1_t12 = isCmc50 ? kof2000_type1_t12 : kof99_type1_t12;
  const a815x1 = isCmc50 ? kof2000_address_8_15_xor1 : kof99_address_8_15_xor1;
  const a815x2 = isCmc50 ? kof2000_address_8_15_xor2 : kof99_address_8_15_xor2;
  const a1623x1 = isCmc50 ? kof2000_address_16_23_xor1 : kof99_address_16_23_xor1;
  const a1623x2 = isCmc50 ? kof2000_address_16_23_xor2 : kof99_address_16_23_xor2;
  const a07 = isCmc50 ? kof2000_address_0_7_xor : kof99_address_0_7_xor;

  const buf = new Uint8Array(romSize);

  // Data XOR pass
  for (let rpos = 0; rpos < romSize / 4; rpos++) {
    const [r0, r3] = cmcDecryptPair(
      rom[4 * rpos]!, rom[4 * rpos + 3]!,
      t0_t03, t0_t12, t1_t03, a07,
      rpos, ((rpos >> 8) & 1) === 1,
    );
    const [r1, r2] = cmcDecryptPair(
      rom[4 * rpos + 1]!, rom[4 * rpos + 2]!,
      t0_t12, t0_t03, t1_t12, a07,
      rpos, (((rpos >> 16) ^ a1623x2[(rpos >> 8) & 0xFF]!) & 1) === 1,
    );
    buf[4 * rpos] = r0;
    buf[4 * rpos + 1] = r1;
    buf[4 * rpos + 2] = r2;
...
export function cmcSfixDecrypt(
  ...
  const src = cRom.subarray(cRomSize - sRomSize);
  for (let i = 0; i < sRomSize; i++) {
    sRom[i] = src[(i & ~0x1F) + ((i & 7) << 2) + ((~i & 8) >> 2) + ((i & 0x10) >> 4)]!;
  }
}
